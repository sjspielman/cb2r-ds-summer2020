...
msleep %>%
  group_by(vore) %>% 
  summarise(max_brainwt = max(brainwt, na.rm = TRUE)) %>%
  arrange(max_brainwt)
``` 
...
11. Use all your skills to wrangle the data to arrive at the answer to this question: Which group has the highest **ratio** of body weight to brain weight: domesticated or non-domesticated mammals? *Make sure to PLAN your steps before coding them. When coding, go LINE by LINE.*
  11 hint: You need to use `mutate()` to create two new columns here: a column containing the ratio of body weight/brain weight, and a column indicating if a mammal is domesticated or not (`if_else()` time!)
...
msleep %>%
  ...
  summarise(ratio_comparison = max(ratio, na.rm = TRUE))
