...
**Hint:** Supply multiple "or" conditions to `filter()` using the logical `%in%` operator, as in: `filter(column %in% c(thing_i_want, other_thing_i_want))`
</div>
...
#### 7. The code below uses a function `tidyr::drop_na()` (a function from the `tidyr` package!) to remove *all rows* of the data that contain an NA. For more flexibility, you can provide a column as an argument to `tidyr::drop_na()` so that only rows with NA's *in that column* will be dropped. 
Run the code below, and then modify it to only remove rows where `conservation` is NA.

```{r, dplyr2_q7, exercise=TRUE}

```

<br>

#### 8. Let's practice *conditional mutation* - creating a new column with `dplyr::mutate()` whose new value should depend on some condition. For example, we may want to do an analysis that compares "heavy" to "light" mammals, where we define "heavy" as weighing 50 or more kg (i.e. 50 kg is considered "heavy"). Use `dplyr::mutate()` with `dplyr::if_else()` to create a new column called `bodywt_class` that defines animals as heavy or light depending on their `bodywt` value.
  
#### The code given below provides an example of using `dplyr::if_else()` with the iris dataset. Use this code as a template to build up your answer.
```{r, dplyr2_q8, exercise=TRUE}
...
iris %>%
 #       new column name              condition             if true    if false
  mutate(is_it_setosa = dplyr::ifelse(Species == "setosa",   "Yes",      "No") )
```

<div id= "dplyr2_q8-hint">
**Hint:** We use `dplyr::if_else()` like this: `dplyr::if_else(logical statement, if TRUE, if FALSE)`
</div> 
...
``` {r, dplyr2_q11-answer}
#Solution to Question 11 (one of many ways to accomplish!)
msleep %>%
  drop_na(conservation) %>%
